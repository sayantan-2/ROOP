...
class Enhance_DMDNet():
    ...
    def enhance_face(self, ref_faceset: FaceSet, temp_frame, face: Face):
        ...
        if self.torchdevice is not None and torch.cuda.is_available():
            LQLocs = LQLocs.to(self.torchdevice)
        ...
        if len(ref_faceset.faces) > 1:
            ...
            for i,face in enumerate(ref_faceset.faces):
                ...
                ref_tensor = read_img_tensor(ref_image, self.torchdevice)
    ...
    def create(self, devicename):
        THREAD_LOCK_DMDNET.acquire()
        try:
            if self.model_dmdnet is None:
                self.model_dmdnet = DMDNet()
                checkpoint = torch.load(resolve_relative_path('../models/DMDNet.pth'), map_location='cpu')
                self.model_dmdnet.load_state_dict(checkpoint['model_state_dict'])
                self.model_dmdnet.eval()

                # Determine device
                self.torchdevice = None
                if torch.cuda.is_available() and not devicename.startswith('cpu'):
                    try:
                        self.torchdevice = 'cuda'
                        self.model_dmdnet = self.model_dmdnet.cuda()
                        print("DMDNet using CUDA")
                    except Exception as e:
                        print(f"Error loading DMDNet to CUDA: {e}")
                        self.torchdevice = 'cpu'
                        print("DMDNet falling back to CPU")
                else:
                    print("DMDNet using CPU")

            return self.model_dmdnet
        finally:
            THREAD_LOCK_DMDNET.release()


def read_img_tensor(Img=None, device=None): #rgb -1~1
...
def get_component_location(Landmarks, re_read=False):
    ...
    L_MO = np.max((np.max(Landmarks[Map_MO],0) - np.min(Landmarks[Map_MO],0))/2,16)) * 1.1
    MO_O = Mean_MO - L_MO + 1
    MO_T = Mean_MO + L_MO
    MO_T[MO_T>510]=510
